...
const Post = ({ post }) => {
  ...
  const handleAddComment = (e) => {
    if (e.keyCode === 13) {
      ...
      http.post(`comment/`, 
        { text: comment.value, post: post.id, user: user.id }, {headers: authHeader()}
      ).then((res) => setNewComments([...newComments, res.data]));
    ...
    <PostWrapper>
      <div className="post-header-wrapper">
        <div className="post-header">
          <Avatar
            ...
            alt="avatar"
            onClick={() => history.push(`/${post.user?.id}`)}
          />
          <h3
            className="pointer"
            onClick={() => history.push(`/${post.user?.id}`)}
      ...
      <div className="likes-caption-comments">
        ...
        <p>
          <span
            onClick={() => history.push(`/${post.user?.id}`)}
            className="pointer nickname bold"
          >
            {post.user?.nickname}
          </span>
          {post.caption}
